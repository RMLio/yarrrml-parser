...
function makeBlankNode(subject, store, writer) {
  let blankNodes = [];
  let triples = store.find(subject, null, null);
  //console.log(triples);
  ...
  triples.forEach(function (triple) {
    ...
    if (N3.Util.isLiteral(triple.object) || ignoredForBlankNodes.indexOf(triple.predicate) !== -1) {
      ...
      if (triple.predicate === namespaces.rr + 'predicateMap') {
        let constants = store.find(triple.object, namespaces.rr + 'constant', null);
...
function replaceTypePOMWithClass(store) {
  let triples = store.find(null, namespaces.rr + 'subjectMap', null);

  triples.forEach(function (triple) {
    let poms = store.find(triple.subject, namespaces.rr + 'predicateObjectMap', null);

    poms.forEach(function (pom) {
      let pms = store.find(pom.object, namespaces.rr + 'predicateMap', null);
      let oms = store.find(pom.object, namespaces.rr + 'objectMap', null);

      //console.log(pms);
      if (pms.length > 0 && oms.length > 0) {
        let types = store.find(pms[0].object, namespaces.rr + 'constant', namespaces.rdf + 'type');
        let constants = store.find(oms[0].object, namespaces.rr + 'constant', null);
...
function removeDefaultTermTypes(store) {
  ...
  triples.forEach(function (triple) {
    store.removeTriples(store.find(triple.object, namespaces.rr + 'termType', namespaces.rr + 'IRI'));
  });

  //remove Literal from objectMap
  triples = store.find(null, namespaces.rr + 'objectMap', null);

  triples.forEach(function (triple) {
    store.removeTriples(store.find(triple.object, namespaces.rr + 'termType', namespaces.rr + 'Literal'));
  });
}

function makeBlankNodesOfObjects(store, writer, subject, predicate) {
  let triples = store.find(subject, predicate, null);
  winston.debug(subject);
  winston.debug(predicate);
  winston.debug(triples);
...
function makeReadableFromArray(rmlTriples, writer) {
  ...
  let triplemaps = store.find(null, namespaces.rr + 'subjectMap', null);
