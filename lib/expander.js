...
function expandChangeDetections (input) {
  if (input.mappings !== undefined && input.mappings !== null) {
    ...
    for (let i = 0; i < mappings.length; i++) {
      ...
      if (mapping) {
        ...
        newMappings = {...newMappings, ...processChangeDetection(mapping, mappingKey)};
      }
    }
    input.mappings = newMappings;
  }
}

function processChangeDetection(mapping, mappingKey) {
  let newMappings = {};
  ...
  if (mapping && mapping.changeDetection) { // ...then we have some InRML stuff here!
    ...
    Object.keys(changeDetection).forEach(operation_name => {
      ...
      if (operation.mappingRemove) {
        processMappingRemove(newMapping, operation.mappingRemove);
      }
    });
  } else {
...
function processMappingRemove(mapping, mappingRemove) {
  ...
  for (let mr of mappingRemove) {
    if (typeof mr === 'string') {
      // If it's a string, then it says which mapping key to remove
      delete mapping[mr];
    } else {
