    ...
    mapping.subjects.forEach(subject => {
      ...
      if (subject.targets) {
        if (Array.isArray(subject.targets)) {
          ...
          for (let i = 0; i < subject.targets.length; i++) {
            ...
            if (Array.isArray(target)) {
              ...
              if (dynTargetMap.has(target)) {
                subject.logicalTargetTemplate = dynTargetMap.get(target);
              } else if (target.includes('$(')){
                subject.logicalTargetTemplate = target;
              }
          ...
          subject.targets = newTargets;
        }
        else if (typeof subject.targets === 'string') {
          if (dynTargetMap.has(subject.targets)) {
            subject.logicalTargetTemplate = dynTargetMap.get(subject.targets);
            delete subject.targets;
          } else {
