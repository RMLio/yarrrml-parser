...
// TODO IncRML: parts of this function might be reused in LDES target
    ...
    if (mapping && mapping.changeDetection) { // ...then we have some InRML stuff here!
      ...
      Object.keys(changeDetection).forEach(operation_name => {
        ...
        if (operation_name === 'delete') {
          // TODO: delete all PO mappings except the ones with predicate `rdf:type`
        }
        ...
        for (const subjectIndex in newMapping.subjects) {
          ...
          const functionName = (isExplicit ? 'explicit' : 'implicit') + operation_name.charAt(0).toUpperCase() + operation_name.slice(1);
          let parameters = [
            {
              parameter: idlabfn + 'iri',
              value: value,
          ...
          if (functionName === 'implicitUpdate' && operation.watchedProperties) {
...
function processMappingRemove(document, mapping, mappingRemove) {
  Logger.debug('Processing mappingRemove...');

  if (typeof mappingRemove === 'string') {
    mappingRemove = mappingRemove === 'po' ? 'predicateobjects' : mappingRemove;
    mappingRemove = mappingRemove === 's' || mappingRemove === 'subject' ? 'subjects' : mappingRemove;
    delete mapping[mappingRemove];
  } else if (typeof mappingRemove === 'object') {
    if (mappingRemove.sources === null) { // This happens when in YA(RRR)ML it says: "sources:" without value.
      delete mapping.sources;
    }
    if (mappingRemove.sources) {
        mapping.sources = mappingRemoveForKey(document, mapping, mappingRemove, 'sources');
    }
    if (mappingRemove.graphs === null) {
      delete mapping.graphs;
    }
    if (mappingRemove.graphs) {
      mapping.graphs = mappingRemoveForKey(document, mapping, mappingRemove, 'graphs');
    }

    replaceAll('predicateobjects', mappingRemove);
    if (mappingRemove.predicateobjects === null) {
      delete mapping.predicateobjects;
    }
    if (mappingRemove.predicateobjects) {
      mappingRemovePO(mapping, mappingRemove);
    }

    replaceAll('subjects', mappingRemove);
    if (mappingRemove.subjects === null) {
      delete mapping.subjects;
    }
    if (mappingRemove.subjects) {
      mappingRemoveSubjects(mapping, mappingRemove);
    }
  }
}
...
function mappingRemovePO(mapping, mappingRemove) {
  // first expand the predicate-objects, so we can compare them to the ones in the original mapping
  expandPredicateObjects(mappingRemove, 'mappingRemove');
...
function processMappingAdd(document, mapping, mappingAdd) {
  //expandPredicateObjects(mappingAdd, 'mappingAdd');

  replaceAll('sources', mappingAdd);
  if (mappingAdd.sources !== undefined && mappingAdd.sources !== null) {
    expandSourcesInMapping(mappingAdd, 'mappingAdd');
    mapping.sources = mappingAddForKey(document, mapping, mappingAdd, 'sources');
  }

  replaceAll('subjects', mappingAdd);
  if (mappingAdd.subjects !== undefined && mappingAdd.subjects !== null) {
    expandSubjects(mappingAdd, 'mappingAdd');
    expandTargetsInMapping(mappingAdd);
    mapping.subjects = mappingAddForKey(document, mapping, mappingAdd, 'subjects');
  }

  replaceAll('graphs', mappingAdd);
  if (mappingAdd.graphs !== undefined && mappingAdd.graphs !== null) {
    expandGraphs(mappingAdd);
    mapping.graphs = mappingAddForKey(document, mapping, mappingAdd, 'graphs');
  }

  replaceAll('predicateobjects', mappingAdd);
  if (mappingAdd.predicateobjects !== undefined && mappingAdd.predicateobjects !== null) {
    expandPredicateObjects(mappingAdd, 'mappingAdd');
    mapping.predicateobjects = addPOMappings(mapping, mappingAdd.predicateobjects);
  }
