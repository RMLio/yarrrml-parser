...
function expandTargetsInDocument(document) {
  ...
  if (document.targets) {
    ...
    for (let i = 0; i < targetKeys.length; i++) {
      ...
      replaceAll('sources', target);

      // The target is dynamic (https://rml.io/specs/target/dynamictarget/) if it has a 'sources' key.
      if (target.sources) {
        // check if target has a variable in its 'access' key. TODO: in future, this can be any key.
        const dynamic_template_var =  target.access.match(/\$\([^\)]*\)/g).join('_');
        // TODO: handle on-match for regex above, a bit like _generateTemplate()
        // generate "unique" id for generated target
        const lt_id_nr = lt_counter++;
        const lt_id_str = ('' + lt_id_nr).padStart(3, '0');
        let generated_dynamic_target_key = 'generated_lt_' + targetName + '_' + dynamic_template_var + '_' + lt_id_str;
        target['generated_dynamic_target_key'] = generated_dynamic_target_key;
        let dynamic_target_key;
        if (target.id) {
          dynamic_target_key = target.id;
        } else {
          // generate "unique" id for generated target
          const lt_id_nr = lt_counter++;
          const lt_id_str = ('' + lt_id_nr).padStart(3, '0');
        ...
        if (target.id) {
          ...
          dynamic_target_key = generated_dynamic_target_key;
        }
        // generate new mappings for dynamic logical target and target
        const dynamic_target_mapping = {
          sources: target.sources,
          subjects: [{
            value: dynamic_target_key,
            thisMappingVar: dynamic_template_var,  // Indicates a dynamic logical target and target has to be generated.
                                                   // This template variable must be used in the name of the targets.
            targets: [target]   // append the original target here, must be used to generate actual target.
          }]
        };
        // add the new mapping:
        const dyn_target_mapping_name = 'dynamic_target_mapping_' + targetName;
        document.mappings[dyn_target_mapping_name] = dynamic_target_mapping;

        // remove the target from the mapping doc.
        delete document.targets[targetName];
        if (Object.keys(document.targets).length === 0) {
          delete document.targets;
        }

        // add mapping <original target name> -> <generated targets>
        dynTargetMap.set(targetName, dynamic_target_key);
      }
...
function expandPredicateObjects(mapping, mappingKey) {
  ...
  if (mapping.predicateobjects) {
    ...
    // check if dynamic target mapping
    if (mapping.subjects.thisMappingVar === null) {
...
function convertArrayTargetInObject(target) {
  ...
  // Serialization can be overriden
  if (target.length > 1) {
...
function expandTermTargets(term){
  replaceAll('targets', term)
  if (term.targets) {
    // multiple dynamic targets are possible per term
    term.logicalTargetTemplates = [];
    if (Array.isArray(term.targets)) {
      const newTargets = Array();
      for (let i = 0; i < term.targets.length; i++) {
        const target = term.targets[i];
        if (Array.isArray(target)) {
          newTargets.push(convertArrayTargetInObject(target));
        } else if (typeof target === 'string') {
          if (dynTargetMap.has(target)) {
            term.logicalTargetTemplates.push(dynTargetMap.get(target));
          } else if (target.includes('$(')) {
            term.logicalTargetTemplates.push(target);
          } else {
      ...
      term.targets = newTargets;
    } else if (typeof term.targets === 'string') {
      if (dynTargetMap.has(term.targets)) {
        term.logicalTargetTemplates.push(dynTargetMap.get(term.targets));
        delete term.targets;
      } else {
        term.targets = [term.targets]
      }
    } else {
      Logger.error(`term "${JSON.stringify(subject, null, 2)}": no (valid) target is defined.`);
    }
  }
}
