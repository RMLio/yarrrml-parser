...
function processMappingRemove(document, mapping, mappingRemove) {
  ...
  if (typeof mappingRemove === 'string') {
    ...
    } else if (mappingRemove.targets) {
      mapping.targets = mappingRemoveForKey(document, mapping, mappingRemove, 'targets');
    }
  }
}
...
function mappingRemoveForKey (document, mapping, mappingRemove, mappingKey) {
  ...
  for (let removeValue of removeValues) {
    ...
    // 'object' in this code is either a source or a target
    for (let objectIndex in mapping[mappingKey]) {
      let object = mapping[mappingKey][objectIndex];
      if (typeof object === 'string') { // && document.sources !== undefined && document.sources !== null) {
