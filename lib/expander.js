  ...
  if (mapping) {
    if (mapping && mapping.changeDetection) { // ...then we have some InRML stuff here!
      ...
      Object.keys(changeDetection).forEach(operation_name => {
        ...
        // TODO: now we know the operation and its type, so we can construct the corresponding IDLab function.
...
function processMappingAdd(document, mapping, mappingAdd) {
  if (mappingAdd.sources) {
    expandSourcesInMapping(mappingAdd, 'mappingAdd');
    mapping.sources = mappingAddSources(document, mapping, mappingAdd, 'sources');
  }
}

function mappingAddSources(document, mapping, mappingAdd) {
  // normalize: convert a string into an array of one string
  const addValues = typeof (mappingAdd.sources) === 'string' ? [mappingAdd.sources] : mappingAdd.sources;
  let newSources = Object.hasOwn(mapping, 'sources')? structuredClone(mapping.sources) : [];
  ...
  for (const addValue of addValues) {
    if (typeof addValue === 'string') {
      ...
      if (document.sources !== undefined && document.sources != null) {
        // it IS a reference!
        const referredSource = document.sources[addValue];
        if (referredSource !== undefined && referredSource !== null) {
          newSources.push(structuredClone(referredSource));
        }
      }
    } else {
      const sourceId = getId(addValue);
      if (sourceId !== undefined) {
        newSources.push(addValue);
      }
    }
  }
  ...
  for (const addValue of addValues) {
    if (typeof addValue === 'object') {
      ...
      if (sourceId === undefined) {
        ...
        for (let sourceIndex in newSources) {
          let source = newSources[sourceIndex];
          if (typeof source === 'string') {
            // if source is a string, then should be a reference to a source. Inline it first
            if (document.sources !== undefined && document.sources != null) {
              // it IS a reference!
              const referredSource = document.sources[source];
              if (referredSource !== undefined && referredSource !== null) {
                source = structuredClone(referredSource);
                newSources[sourceIndex] = source;
              }
            }
          }
          for (const key of Object.keys(addValue)) {
            source[key] = addValue[key];
          }
        }
      }
    }
