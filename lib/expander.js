...
function processMappingRemove(document, mapping, mappingRemove) {
  ...
  if (typeof mappingRemove === 'string') {
    ...
    if (mappingRemove.sources) {
      ...
      const removeValues = typeof (mappingRemove.sources) === 'string' ? [mappingRemove.sources] : mappingRemove.sources;
      let newSources = [];

      for (let removeValue of removeValues) {
          // Iterate over the mapping's sources and adjust.
          // Note that inlining a referred source is important because other mappings may also refer to that source!
          for (let sourceIndex in mapping.sources) {

            let source = mapping.sources[sourceIndex];
            if (typeof source === 'string') { // && document.sources !== undefined && document.sources !== null) {
              // is 'removeValue' the source name by coincidence?
              if (source !== removeValue) {
                // it's a reference! so make it inline and process as if it's inline.
                // Find the source, if any.
                if (document.sources !== undefined && document.sources !== null) {
                  ...
                  if (referredSource !== undefined && referredSource !== null) {
                    ...
                    delete source[removeValue];
                    // Inline referred source (= replace reference with copy of original)
