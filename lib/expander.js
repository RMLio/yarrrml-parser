...
function mappingRemoveForKey (document, mapping, mappingRemove, mappingKey) {
  ...
  for (const removeValue of removeValues) {
    // Iterate over the mapping's [sources or targets] (called 'objects' from now on) and adjust.
...
function getId(termMap) {
  // an object mapping gets identified by a `value` or a `function` or ....
  let id;
...
function mappingRemoveSubjects(mapping, mappingRemove) {
  ...
  let newSubjects = [];
  for (const subject of mappingRemove.subjects) {
    ...
    for (const mappingSubject of mapping.subjects) {
      ...
      if (subjectId === mappingSubjectId) {
        // TODO: remove targets here if defined
        // TODO: specify: how to remove only targets? Or is that not necessary?
      } else {
