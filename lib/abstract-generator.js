...
class AbstractGenerator {
  ...
  generateMapping(tmSubject, mapping, mappingName, sourceSubject, targetsIRIMap) {
    ...
    mapping.subjects.forEach(subject => {
      ...
      this.quads.push(quad(
        ...
        namedNode(getNamespace('rr') + 'SubjectMap')
      ));
      ...
      if (typeof subject === "object") {
        if (subject.function) {
          ...
          this.quads.push(quad(
            smSubject,
            namedNode(getNamespace('rr') + 'termType'),
            namedNode(getNamespace('rr') + 'BlankNode')
          ));
        }
      } else {
      ...
      if (subject.targets) {
        subject.targets.forEach(target => {
          this.quads.push(quad(
            smSubject,
            namedNode(getNamespace('rml') + 'logicalTarget'),
    ...
    if (mapping.predicateobjects) {
      mapping.predicateobjects.forEach(po => {
        ...
        if (this.options.class && // flag
          ...
          subjectMaps.forEach(sMap => {
            po.objects.forEach(o => {
              ...
              this.quads.push(quad(
                sMap,
                namedNode(getNamespace('rr') + 'class'),
                object
              ));
            });
          });
        ...
        this.quads.push(quad(
          pomSubject,
          namedNode(getNamespace('rdf') + 'type'),
          namedNode(getNamespace('rr') + 'PredicateObjectMap')
        ));
        ...
        po.predicates.forEach(p => {
          ...
          this.quads.push(quad(
            pmSubject,
            namedNode(getNamespace('rdf') + 'type'),
            namedNode(getNamespace('rr') + 'PredicateMap')
          ));
          ...
          if (p === 'a') {
            appropriatePO = {
              object: namedNode(getNamespace('rdf') + 'type'),
              predicate: namedNode(getNamespace('rr') + 'constant')
            };
        ...
        po.objects.forEach(o => {
          ...
          this.quads.push(quad(
            pomSubject,
            namedNode(getNamespace('rr') + 'objectMap'),
            omSubject
          ));
  ...
  generateNormalObjectMap(omSubject, o, targetsIRIMap) {
    ...
    switch (o.type) {
      case 'iri':
        o.type = namedNode(getNamespace('rr') + 'IRI');
        break;
      case 'blank':
        o.type = namedNode(getNamespace('rr') + 'BlankNode');
        break;
      case 'literal':
      default:
        o.type = namedNode(getNamespace('rr') + 'Literal');
        break;
    }
  ...
  generateReferencingObjectMap(omSubject, o) {
    ...
    this.quads.push(quad(
      omSubject,
      namedNode(getNamespace('rr') + 'parentTriplesMap'),
  ...
  generateCondition(condition, omSubject) {
    ...
    this.quads.push(quad(
      omSubject,
      namedNode(getNamespace('rr') + 'joinCondition'),
    ...
    condition.parameters.forEach(parameter => {
      ...
      if (parameter.from === 'subject') {
        predicate = namedNode(getNamespace('rr') + 'child');
      } else {
        predicate = namedNode(getNamespace('rr') + 'parent');
      }
  ...
  generateFunctionTermMap(omSubject, o, sourceSubject, termType, targetsIRIMap) {
    ...
    this.quads.push(quad(
      omSubject,
      namedNode(getNamespace('rdf') + 'type'),
      namedNode(getNamespace('fnml') + 'FunctionTermMap')
    ));
    ...
    this.quads.push(quad(
      omSubject,
      namedNode(getNamespace('fnml') + 'functionValue'),
      fnSubject
    ));
    ...
    this.quads.push(quad(
      pmExecutesSubject,
      namedNode(getNamespace('rr') + 'constant'),
      namedNode(getNamespace('fno') + 'executes')
    ));
        ...
        if (pm.from === 'subject') {
          if (pm.type === 'blank') {
            ...
            this.quads.push(quad(
              omSubject,
              namedNode(getNamespace('rr') + 'termType'),
              namedNode(getNamespace('rr') + termTypeObject)
            ));
          }
          ...
          this.quads.push(quad(
            omSubject,
            namedNode(getNamespace('rml') + 'parentTermMap'),
  ...
  generateGraphMap(subject, graph, sourceSubject, targetsIRIMap) {
    ...
    this.quads.push(quad(
      subject,
      namedNode(getNamespace('rr') + 'graphMap'),
      graphMapSubject
    ));
  ...
  _generateDatasetDescription(authors) {
    ...
    for (const author of authors) {
      if (author.webid) {
        this.quads.push(quad(
          this.datasetIRI,
          namedNode(getNamespace('dcterms') + 'contributor'),
        ...
        this.quads.push(quad(
          personIRI,
          namedNode(getNamespace('dcterms') + 'contributor'),
          namedNode(getNamespace('foaf') + 'Person')
        ));
        ...
        if (author.email) {
          this.quads.push(quad(
            personIRI,
            namedNode(getNamespace('foaf') + 'mbox'),
        ...
        if (author.website) {
          this.quads.push(quad(
            personIRI,
            namedNode(getNamespace('foaf') + 'homepage'),
  ...
  addMappingIRI(mappingName, iri) {
    ...
    if (this.datasetIRI) {
      ...
      this.quads.push(quad(
        this.datasetIRI,
        namedNode(getNamespace('void') + 'exampleResource'),
        iri
      ));
    }
  }
  ...
  getAppropriatePredicateAndObjectForValue(value, isIRI = false) {
    let predicate = namedNode(getNamespace('rr') + 'template');
  ...
  processDatatypeAndLanguageOfObject(o, omSubject, targetsIRIMap) {
    ...
    if (o.datatype) {
      this.quads.push(quad(
        omSubject,
        namedNode(getNamespace('rr') + 'datatype'),
