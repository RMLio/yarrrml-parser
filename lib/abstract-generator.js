    ...
    mapping.subjects.forEach(subject => {
      ...
      if (typeof subject === "object") {
        if (subject.function) {
          ...
          if (subject.logicalTargetTemplate) {
            ...
            // add the template
            const {predicate, object} = this.getAppropriatePredicateAndObjectForValue(subject.logicalTargetTemplate);
            this.quads.push(quad(
                ltSubject,
                predicate,
                object
            ));
            // add the logical target map to the subject map
            this.quads.push(quad(
                smSubject,
                namedNode(namespaces.rmle + 'LogicalTargetMap'),
                ltSubject
