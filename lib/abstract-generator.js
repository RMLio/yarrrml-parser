...
class AbstractGenerator {
  ...
  convert(yarrrml) {
    ...
    try {
      ...
      if (expandedJSON.external) {
        this.externalReferences = expandedJSON.external;
      }
  ...
  _replaceExternalReferences(str) {
    ...
    for (const ref of refs) {
      if (ref.startsWith('{_')) {
        ...
        str = str.replace(ref, '{' + ref.substr(2));
      }
    }
  ...
  getAppropriatePredicateAndObjectForValue(value, isIRI = false) {
    ...
    if (AbstractGenerator.countReference(parsedValue) === 1 && !AbstractGenerator.hasConstantPart(parsedValue)) {
      ...
      const temp = this._replaceExternalReferences(AbstractGenerator.expandPrefix(parsedValue));
      object = literal(temp);
    }
