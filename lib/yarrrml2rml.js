...
class YARRRML2RML extends YARRRML2Anything {
  ...
  convertExpandedJSON(yarrrml) {
    ...
    let baseIRI = this.baseIRI = yarrrml.base;
    const sourcesIRIMap = {};
    ...
    Object.keys(yarrrml.mappings).forEach(mappingName => {
      ...
      if (mapping.sources) {
        ...
        mapping.sources.forEach(source => {
          const tmSubject = baseIRI + this.getUniqueID('map_' + mappingName);
        ...
        this.generateMapping(triples, tmSubject, mapping, baseIRI, mappingName);
      }
    });

    this.generateAllReferencingObjectMap(triples, baseIRI);
    return triples;
  }
