...
describe('RML2YARRRML', () => {
  ...
  it('Works for rml2yarrrml_1', async () => {
    const str = await toYARRRMLFromFile(path.resolve(__dirname, '../test/rml2yarrrml_1/mapping_reverse.ttl'));
  ...
  it('Works for rml2yarrrml_2', async () => {
    const str = await toYARRRMLFromFile(path.resolve(__dirname, '../test/rml2yarrrml_2/mapping.rml.ttl'));
    const output = fs.readFileSync(path.resolve(__dirname, '../test/rml2yarrrml_2/mapping.yarrrml.yml'), 'utf8');
    assert.strictEqual(str.replace(/\r\n|\r|\n/g, '\n'), output.replace(/\r\n|\r|\n/g, '\n'));
  });
});
