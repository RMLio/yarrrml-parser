    ...
    o.parameters.forEach(pm => {
      ...
      if (pm.from === 'subject') {
        if (pm.value instanceof Object) {
          ...
          if (YARRRML2Anything.countReference(object) === 1) {
            object = `"${YARRRML2Anything.getFirstReference(object)}"`;
        ...
        const parentTermMapSubject = this.baseIRI + this.getUniqueID('ptm');

        this.triples.push({
          subject: omSubject,
          predicate: namespaces.rml + 'parentTermMap',
          object: parentTermMapSubject
        });

        let object = `"${YARRRML2Anything.expandPrefix(YARRRML2Anything.parseTemplate(pm.value))}"`;
        let predicate = namespaces.rr + 'template';
