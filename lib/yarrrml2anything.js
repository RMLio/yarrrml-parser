...
class YARRRML2Anything {
    ...
    if (mapping.subjects) {
      mapping.subjects.forEach(subject => {
        ...
        if (typeof subject === "object") {
          ...
          this.quads.push(quad(
            smSubject,
            namedNode(namespaces.rr + 'template'),
            literal(YARRRML2Anything.expandPrefix(YARRRML2Anything.parseTemplate(subject)))
          ));
        }
  ...
  getAppropriatePredicateAndObjectForObjectValue(value) {
    ...
    const parsedValue = YARRRML2Anything.parseTemplate(value);
    let object = literal(parsedValue);

    if (YARRRML2Anything.countReference(parsedValue) === 1) {
