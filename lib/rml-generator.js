...
class RMLGenerator extends YARRRML2Anything {
  ...
  _parametersContainsConstantValues(parameters) {
    ...
    (typeof parameters[i].value !== 'string' || (typeof parameters[i].value === 'string' && this.getAppropriatePredicateAndObjectForValue(parameters[i].value).predicate.value !== namespaces.rr + 'constant'))) {
      i ++
    }
