...
class RMLGenerator extends AbstractGenerator {
  ...
  _generateTargetId(target) {
    if (typeof target === 'object') {
      let id = this._generateTargetObjectIdStr(target);
    ...
    /**
     * Recursive formatting of (objects in) a target
     * @param input The target or object in a target to format
     * @returns {string} The objects' keys and values as string, separated with '-'
     * @private
     */
  _generateTargetObjectIdStr(input) {
    let id = '';
    Object.keys(input).forEach(key => {
      if (key === 'ldes') {
        ...
        if (typeof value === 'object') {
          id += this._generateTargetObjectIdStr(value);
        } else {
          id += input[key];
        }
      }
  ...
  generateTargetTriplesMap(target, logicalTargetTemplate, thisMappingVar, mappingName, sourcesIRIMap) {
    ...
    if (target.type === 'void') {
      ...
      // check for optional 'headers' key
      if (target.headers) {
        const listNode = this._generateHeaderList(target.headers);
        this._generatePOMap(tSubject, namespaces.htv + "headers", {value: listNode.value, type: 'iri'});
      }
  ...
  _generateHTTPRequestAccess(sourceOrTarget, sourceOrTargetSubject, sourceOrTargetName, isSource, isDirect) {
    ...
    // check for optional headers (if target)
    if (!isSource) {
      if (sourceOrTarget.headers) {
        const listNode = this._generateHeaderList(sourceOrTarget.headers);
        this.quads.push(quad(
          httpSubject,
          namedNode(namespaces.htv + 'headers'),
          listNode
        ));
      }
    }
  ...
  /**
   * Generates a PO Map from a single predicate and a single object.
  ...
  _generatePOMap(tmSubject, predicate, object) {
    ...
    /**
     *
     * @param {{}} headers An object containing HTTP header names and their value as key-values.
     * @returns {BlankNode|NamedNode} The node represting the list element
     * @private
     */
    _generateHeaderList(headers) {
        const headerNodes = [];
        Object.entries(headers).forEach(
            ([header, value]) => {
                // create a 'header' node and add it to the list
                const headerSubject = namedNode(this.baseIRI + 'header-' + this.getUniqueID(header));
                headerNodes.push(headerSubject);

                // Add the quads of the header node to the resulting quads
                this.quads.push(quad(
                    headerSubject,
                    namedNode(namespaces.rr + 'class'),
                    namedNode(namespaces.htv + 'RequestHeader')
                ));
                this.quads.push(quad(
                    headerSubject,
                    namedNode(namespaces.htv + 'fieldName'),
                    literal(header)
                ));
                this.quads.push(quad(
                    headerSubject,
                    namedNode(namespaces.htv + 'fieldValue'),
                    literal(value)
                ));
            }
        );

        // Create a list of the header nodes
        const {listElement, listQuads} = this.utils.createList(headerNodes, this.getBaseIRI());
        this.quads.push(...listQuads);
