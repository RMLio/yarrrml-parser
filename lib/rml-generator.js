...
class RMLGenerator extends AbstractGenerator {
        ...
        mapping.sources.forEach(source => {
          ...
          if (typeof source === 'string') {
            ...
            this.quads.push(quad(
              tmSubject,
              namedNode(getNamespace('rml') + 'logicalSource'),
              sourceSubject
            ));
          } else {
  ...
  generateSource(source, tmSubject, sourceName) {
    ...
    if (!source.type) {
      if (source.referenceFormulation === 'csv' && source.delimiter !== undefined && source.delimiter !== ',') {
       // We need CSVW.
        this.prefixes['csvw'] = getNamespace('csvw');
        ...
        this.quads.push(quad(
          sSubject,
          namedNode(getNamespace('rml') + 'source'),
          csvwSubject
        ));
        ...
        this.quads.push(quad(
          csvwSubject,
          namedNode(getNamespace('rdf') + 'type'),
          namedNode(getNamespace('csvw') + 'Table')
        ));

        this.quads.push(quad(
          csvwSubject,
          namedNode(getNamespace('csvw') + 'url'),
          literal(source.access)
        ));

        this.quads.push(quad(
          csvwSubject,
          namedNode(getNamespace('csvw') + 'dialect'),
          dialectSubject
        ));
        ...
        this.quads.push(quad(
          dialectSubject,
          namedNode(getNamespace('csvw') + 'delimiter'),
      ...
      if (source.iterator) {
        this.quads.push(quad(
          sSubject,
          namedNode(getNamespace('rml') + 'iterator'),
      ...
      if (source.type == 'wot') {
        ...
        this.prefixes['td'] = getNamespace('td')
        this.prefixes['wotsec'] = getNamespace('wotsec')
        this.prefixes['hctl'] = getNamespace('hctl')
        this.prefixes['idsa'] = getNamespace('idsa')
        ...
        this.quads.push(quad(
          wotSubject,
          namedNode(getNamespace('rdf') + 'type'),
          namedNode(getNamespace('td') + 'Thing')
        ));
        ...
        this.quads.push(quad(
          propertyAffordanceSubject,
          namedNode(getNamespace('rdf') + 'type'),
          namedNode(getNamespace('td') + 'PropertyAffordance')
        ));

        this.quads.push(quad(
          propertyAffordanceSubject,
          namedNode(getNamespace('td') + 'hasForm'),
          formSubject
        ));
        ...
        this.quads.push(quad(
          formSubject,
          namedNode(getNamespace('rdf') + 'type'),
          namedNode(getNamespace('td') + 'Form')
        ));

        this.quads.push(quad(
          formSubject,
          namedNode(getNamespace('hctl') + 'hasTarget'),
          literal(source.access)
        ));

        this.quads.push(quad(
          formSubject,
          namedNode(getNamespace('hctl') + 'forContentType'),
        ...
        switch(source.operationType) {
          case 'read':
            operationType = namedNode(getNamespace('td') + 'readproperty');
            break
        ...
        this.quads.push(quad(
          formSubject,
          namedNode(getNamespace('hctl') + 'hasOperationType'),
          operationType
        ));

        this.quads.push(quad(
          wotSubject,
          namedNode(getNamespace('td') + 'hasPropertyAffordance'),
          propertyAffordanceSubject
        ));
        ...
        if (source.security) {
            ...
            Object.keys(source.security).forEach(index => {
              ...
              if (security.type == 'apikey') {
                this.quads.push(quad(
                  securitySubject,
                  namedNode(getNamespace('rdf') + 'type'),
                  namedNode(getNamespace('td') + 'APISecurityScheme')
                ));

                this.quads.push(quad(
                  securitySubject,
                  namedNode(getNamespace('wotsec') + 'in'),
                  literal(security.in)
                ));

                this.quads.push(quad(
                  securitySubject,
                  namedNode(getNamespace('wotsec') + 'name'),
                  literal(security.name)
                ));

                this.quads.push(quad(
                  securitySubject,
                  namedNode(getNamespace('idsa') + 'tokenValue'),
                ...
                this.quads.push(quad(
                  securitySubject,
                  namedNode(getNamespace('rdf') + 'type'),
                  namedNode(getNamespace('td') + 'NoSecurityScheme')
                ));
              }
            });
          }
          ...
          this.quads.push(quad(
            wotSubject,
            namedNode(getNamespace('td') + 'hasSecurityConfiguration'),
            securitySubject
          ));
      ...
      /* Database */
        ...
        if (source.query) {
          this.quads.push(quad(
            sSubject,
            namedNode(getNamespace('rml') + 'query'),
    ...
    this.quads.push(quad(
      sSubject,
      namedNode(getNamespace('rml') + 'referenceFormulation'),
      object
    ));
    ...
    this.prefixes['rmlt'] = getNamespace('rmlt');

    this.quads.push(quad(
      tSubject,
      namedNode(getNamespace('rdf') + 'type'),
      namedNode(getNamespace('rmlt') + 'LogicalTarget')
    ));
    ...
    let format = namedNode(getNamespace('formats') + 'N-Quads');
    this.prefixes['formats'] = getNamespace('formats');

    if (target.serialization) {
      switch (target.serialization) {
        case 'jsonld':
          format = namedNode(getNamespace('formats') + 'JSON-LD');
          break;
        case 'n3':
          format = namedNode(getNamespace('formats') + 'N3');
          break;
        case 'ntriples':
          format = namedNode(getNamespace('formats') + 'N-Triples');
          break;
        case 'nquads':
          format = namedNode(getNamespace('formats') + 'N-Quads');
          break;
        case 'ldpatch':
          format = namedNode(getNamespace('formats') + 'LD_Patch');
          break;
        case 'microdata':
          format = namedNode(getNamespace('formats') + 'microdata');
          break;
        case 'owlxml':
          format = namedNode(getNamespace('formats') + 'OWL_XML');
          break;
        case 'owlfunctional':
          format = namedNode(getNamespace('formats') + 'OWL_Functional');
          break;
        case 'owlmanchester':
          format = namedNode(getNamespace('formats') + 'OWL_Manchester');
          break;
        case 'powder':
          format = namedNode(getNamespace('formats') + 'POWDER');
          break;
        case 'powder-s':
          format = namedNode(getNamespace('formats') + 'POWDER-S');
          break;
        case 'prov-n':
          format = namedNode(getNamespace('formats') + 'PROV-N');
          break;
        case 'prov-xml':
          format = namedNode(getNamespace('formats') + 'PROV-XML');
          break;
        case 'rdfa':
          format = namedNode(getNamespace('formats') + 'RDFa');
          break;
        case 'rdfjson':
          format = namedNode(getNamespace('formats') + 'RDF_JSON');
          break;
        case 'rdfxml':
          format = namedNode(getNamespace('formats') + 'RDF_XML');
          break;
        case 'rifxml':
          format = namedNode(getNamespace('formats') + 'RIF_XML');
          break;
        case 'sparqlxml':
          format = namedNode(getNamespace('formats') + 'SPARQL_Results_XML');
          break;
        case 'sparqljson':
          format = namedNode(getNamespace('formats') + 'SPARQL_Results_JSON');
          break;
        case 'sparqlcsv':
          format = namedNode(getNamespace('formats') + 'SPARQL_Results_CSV');
          break;
        case 'sparqltsv':
          format = namedNode(getNamespace('formats') + 'SPARQL_Results_TSV');
          break;
        case 'turtle':
          format = namedNode(getNamespace('formats') + 'Turtle');
          break;
        case 'trig':
          format = namedNode(getNamespace('formats') + 'TriG');
          break;
        default:
          format = namedNode(getNamespace('formats') + 'N-Quads');
          break;
    ...
    this.quads.push(quad(
      tSubject,
      namedNode(getNamespace('rmlt') + 'serialization'),
      format
    ));
    ...
    if (target.compression) {
      ...
      this.prefixes['comp'] = getNamespace('comp');

      if (algorithms.indexOf(target.compression) > -1) {
        this.quads.push(quad(
          tSubject,
          namedNode(getNamespace('rmlt') + 'compression'),
          namedNode(getNamespace('comp') + target.compression)
        ));
      }
      else {
    ...
    if (!target.type) {
      ...
      this.prefixes['void'] = getNamespace('void');

      this.quads.push(quad(
        tSubject,
        namedNode(getNamespace('rmlt') + 'target'),
        voidSubject
      ));
      ...
      this.quads.push(quad(
        voidSubject,
        namedNode(getNamespace('rdf') + 'type'),
        namedNode(getNamespace('void') + 'Dataset')
      ));

      this.quads.push(quad(
        voidSubject,
        namedNode(getNamespace('void') + 'dataDump'),
    ...
    /* SPARQL endpoint */
      ...
      this.prefixes['sd'] = getNamespace('sd');
      ...
      this.quads.push(quad(
        sdSubject,
        namedNode(getNamespace('rdf') + 'type'),
        namedNode(getNamespace('sd') + 'Service')
      ));

      this.quads.push(quad(
        sdSubject,
        namedNode(getNamespace('sd') + 'supportedLanguage'),
        namedNode(getNamespace('sd') + 'SPARQL11Update')
      ));

      if (target.access) {
        this.quads.push(quad(
          sdSubject,
          namedNode(getNamespace('sd') + 'endpoint'),
    ...
    /* DCAT Dataset */
      ...
      this.prefixes['dcat'] = getNamespace('dcat');
      ...
      this.quads.push(quad(
        dcatSubject,
        namedNode(getNamespace('rdf') + 'type'),
        namedNode(getNamespace('dcat') + 'Dataset')
      ));

      if (target.access) {
        this.quads.push(quad(
          dcatSubject,
          namedNode(getNamespace('dcat') + 'dataDump'),
          namedNode('file://' + target.access)
        ));
      }
      else {
    ...
    // Add LDES specific triples, if any
      if (target.ldes) {
        this.prefixes['ldes'] = getNamespace('ldes');
        this.prefixes['tree'] = getNamespace('tree');

        // say it's an LDES logical target
        this.quads.push(quad(
            tSubject,
            namedNode(getNamespace('rdf') + 'type'),
            namedNode(getNamespace('ldes') + 'EventStreamTarget'),
        ));

        // add the required LDES "base" iri
        this.quads.push(quad(
            tSubject,
            namedNode(getNamespace('ldes') + 'baseIRI'),
        ...
        this.quads.push(quad(
            tSubject,
            namedNode(getNamespace('tree') + 'shape'),
        ...
        if (target.ldes.timestampPath) {
          this.quads.push(quad(
              tSubject,
              namedNode(getNamespace('ldes') + 'timestampPath'),
        ...
        if (target.ldes.versionOfPath) {
          this.quads.push(quad(
              tSubject,
              namedNode(getNamespace('ldes') + 'versionOfPath'),
  ...
  _generateDatabaseDescription(subject, source) {
    this.quads.push(quad(
      subject,
      namedNode(getNamespace('rdf') + 'type'),
      namedNode(getNamespace('d2rq') + 'Database')
    ));

    this.quads.push(quad(
      subject,
      namedNode(getNamespace('d2rq') + 'jdbcDSN'),
      literal(source.access)
    ));

    this.quads.push(quad(
      subject,
      namedNode(getNamespace('d2rq') + 'jdbcDriver'),
    ...
    if (source.credentials) {
      if (source.credentials.username) {
        this.quads.push(quad(
          subject,
          namedNode(getNamespace('d2rq') + 'username'),
      ...
      if (source.credentials.password) {
        this.quads.push(quad(
          subject,
          namedNode(getNamespace('d2rq') + 'password'),
  ...
  generateLanguageTerms(objectMap, value) {
    ...
    this.quads.push(quad(
      objectMap,
      namedNode(getNamespace('rml') + 'languageMap'),
      languageMapSubject
    ));
  ...
  generateCondition(condition, omSubject) {
    if (condition.function === 'equal'
      ...
      this.quads.push(quad(
        omSubject,
        namedNode(getNamespace('rml') + 'joinCondition'),
  ...
  getReferenceOnlyPredicate() {
    return namedNode(getNamespace('rml') + 'reference');
  }
  ...
  _parametersContainsConstantValues(parameters) {
    ...
    (typeof parameters[i].value !== 'string' ||
      (typeof parameters[i].value === 'string'
        && this.getAppropriatePredicateAndObjectForValue(parameters[i].value).predicate.value !== getNamespace('rr') + 'constant'))) {
      i ++
    }
  ...
  _parametersContainsTemplates(parameters) {
    ...
    (typeof parameters[i].value !== 'string' ||
      (typeof parameters[i].value === 'string'
        && this.getAppropriatePredicateAndObjectForValue(parameters[i].value).predicate.value !== getNamespace('rr') + 'template'))) {
      i ++
    }
