...
class RMLGenerator extends AbstractGenerator {
  ...
  generateTargetTriplesMap(target, logicalTargetTemplate, thisMappingVar, mappingName, sourcesIRIMap) {
    ...
    const targetTemplate = logicalTargetTemplate.slice(1);
    this._generatePOMap(tmSubject, namespaces.rml + "target", {value: targetTemplate, type: "literal"});
    ...
    this.quads.push(quad(
        sSubject,
        namedNode(namespaces.rml + 'class'),
        namedNode(namespaces.rmlt + 'Target')
    ));
